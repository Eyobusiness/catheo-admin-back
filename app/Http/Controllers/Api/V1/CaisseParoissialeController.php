...
class CaisseParoissialeController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $mouvements = $query->latest('date_mouvement')->paginate($request->integer('per_page', 15));

        // Calculs des KPIs conforme à l'image 2
        $totalEncaisse = (float) CaisseParoissiale::where('paroisse_configuration_id', $paroisseId)
            ->whereIn('type_mouvement', ['entree', 'recette'])
            ->sum('montant');

        $totalRembourse = (float) CaisseParoissiale::where('paroisse_configuration_id', $paroisseId)
            ->whereIn('type_mouvement', ['sortie', 'depense', 'remboursement'])
            ->sum('montant');

        $soldeCaisse = $totalEncaisse - $totalRembourse;

        $paiementsValidesCount = Paiement::where('paroisse_configuration_id', $paroisseId)
            ->where('statut', 'valide')
            ->count();
