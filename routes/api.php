...
use App\Http\Controllers\Api\V1\ParoisseConfigurationController;
use App\Http\Controllers\Api\V1\ApparenceConfigurationController;
use App\Http\Controllers\Api\V1\ResponsableParoisseController;
...
use App\Http\Controllers\Api\V1\VersementCureController;
use App\Http\Controllers\Api\V1\DonCotisationController;
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    // ─────────────────────────────────────────────────────────────────
    // PHASE 2 — Paramètres Paroisse, Apparence & Sauvegardes
    // Permissions : settings.manage
    // ─────────────────────────────────────────────────────────────────
    Route::get('/paroisse-configuration', [ParoisseConfigurationController::class, 'show']);
    Route::put('/paroisse-configuration', [ParoisseConfigurationController::class, 'update'])
    ...
    Route::middleware('permission:settings.manage')->group(function () {
        ...
        Route::apiResource('responsables-paroisse', ResponsableParoisseController::class);
    });
    ...
    Route::middleware('permission:catechumenes.view')->group(function () {
        Route::apiResource('campagnes-preinscriptions', CampagnePreinscriptionController::class)->only(['index', 'show']);
        ...
        Route::apiResource('inscriptions-annuelles', InscriptionAnnuelleController::class)->only(['index', 'show']);
    ...
    Route::middleware('permission:catechumenes.create')->group(function () {
        Route::apiResource('campagnes-preinscriptions', CampagnePreinscriptionController::class)->only(['store']);
        ...
        Route::apiResource('inscriptions-annuelles', InscriptionAnnuelleController::class)->only(['store']);
    ...
    Route::middleware('permission:catechumenes.edit')->group(function () {
        Route::apiResource('campagnes-preinscriptions', CampagnePreinscriptionController::class)->only(['update']);
        ...
        Route::apiResource('inscriptions-annuelles', InscriptionAnnuelleController::class)->only(['update']);
    ...
    Route::middleware('permission:catechumenes.delete')->group(function () {
        Route::apiResource('campagnes-preinscriptions', CampagnePreinscriptionController::class)->only(['destroy']);
        Route::delete('/catechumenes/{catechumene}', [CatechumeneController::class, 'destroy']);
        Route::apiResource('inscriptions-annuelles', InscriptionAnnuelleController::class)->only(['destroy']);
    ...
    Route::middleware('permission:finances.view')->group(function () {
        ...
        Route::apiResource('versements-cure', VersementCureController::class)->only(['index', 'show']);
        Route::apiResource('operations-paiements', OperationPaiementController::class)->only(['index', 'show']);
    });

    Route::middleware('permission:finances.create')->group(function () {
        Route::post('/paiements', [PaiementController::class, 'store']);
        Route::apiResource('tarifs', TarifController::class)->only(['store']);
        Route::apiResource('caisse-paroissiale', CaisseParoissialeController::class)->only(['store']);
        Route::apiResource('versements-cure', VersementCureController::class)->only(['store']);
        Route::apiResource('operations-paiements', OperationPaiementController::class)->only(['store']);
        Route::apiResource('don-cotisations', DonCotisationController::class)->only(['store']);
    });

    Route::middleware('permission:finances.edit')->group(function () {
        Route::apiResource('tarifs', TarifController::class)->only(['update']);
        Route::apiResource('versements-cure', VersementCureController::class)->only(['update']);
        Route::apiResource('operations-paiements', OperationPaiementController::class)->only(['update']);
        Route::apiResource('don-cotisations', DonCotisationController::class)->only(['update']);
    });
    ...
    Route::middleware('permission:finances.delete')->group(function () {
        ...
        Route::apiResource('tarifs', TarifController::class)->only(['destroy']);
        Route::apiResource('versements-cure', VersementCureController::class)->only(['destroy']);
        Route::apiResource('operations-paiements', OperationPaiementController::class)->only(['destroy']);
        Route::apiResource('don-cotisations', DonCotisationController::class)->only(['destroy']);
    });
    ...
    Route::prefix('impressions')->group(function () {
        Route::get('/entete', [ImpressionController::class, 'entete']);
        Route::post('/fiche-notes', [ImpressionController::class, 'ficheNotes']);
        Route::post('/fiche-presences', [ImpressionController::class, 'fichePresences']);
        Route::post('/liste-presence', [ImpressionController::class, 'listePresence']);
        Route::post('/liste-catechumenes', [ImpressionController::class, 'listeCatechumenes']);
        Route::post('/suivi-sacramental', [ImpressionController::class, 'suiviSacramental']);
        Route::post('/fiche-bilan-annuel', [ImpressionController::class, 'ficheBilanAnnuel']);
        Route::post('/fiche-renseignement-bapteme', [ImpressionController::class, 'ficheRenseignementBapteme']);
        Route::post('/fiche-renseignement-premiere-communion', [ImpressionController::class, 'ficheRenseignementPremiereCommunion']);
        Route::post('/fiche-renseignement-confirmation', [ImpressionController::class, 'ficheRenseignementConfirmation']);
    });
